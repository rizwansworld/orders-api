...
import (
	...
	"github.com/rizwansworld/orders-api/model"
)
...
func (r *RedisRepo) Insert(ctx context.Context, order model.Order) error {
	...
	// only using Set() will overwrite the existing data
	// using SetNX() will not overwrite if data exists, it will just return an error.
...
func (r *RedisRepo) FindAll(ctx context.Context, page FindAllPage) (FindResult, error) {
	...
	// Returning empty list as response if no keys are returned from set
	if len(keys) == 0 {
		return FindResult{
			Orders: []model.Order{},
		}, nil
